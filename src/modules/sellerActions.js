...
const changePrice = async (req, res) => {
  ...
  try {
    const itemWithUpdatedPrice = await Item.findOneAndUpdate(
      { _id: itemId, sellerId },
      { price: newPrice },
      { new: true }
    )
    if (!itemWithUpdatedPrice) {
      return res
        .status(404)
        .send('Unable to find an item with that combination of itemId/sellerId')
    }
    res.json(itemWithUpdatedPrice)
  } catch (err) {
    console.log(
      `Failed to update price for itemId ${itemId} with message: ${err.message}`
    )
